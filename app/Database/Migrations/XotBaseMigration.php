...
abstract class XotBaseMigration extends LaravelMigration
{
    ...
    /**
     * Vincolo di foreign key presente sulla tabella corrente.
     *
     * Serve nel ramo `tableUpdate()`: quando una CREATE passa ma la FK fallisce
     * (tipi non combacianti, errno 150), la tabella resta senza vincolo e
     * `tableCreate()` non la ritocca piu' perche' ormai esiste. Riaggiungere la
     * FK senza guardia darebbe "Duplicate key name" al secondo giro.
     *
     * @param  string  $constraint  Nome del vincolo, es. `team_permissions_team_id_foreign`
     */
    public function hasForeignKey(string $constraint): bool
    {
        $rows = $this->getConn()->getConnection()->select(
            'SELECT 1
              ...
              WHERE table_schema = DATABASE()
                ...
                AND constraint_type = ?
              LIMIT 1',
            [$this->getTable(), $constraint, 'FOREIGN KEY'],
        );
    ...
    /**
     * Rollback: volutamente nessuna operazione.
     *
     * I dati sono sacri e le migrazioni di questo progetto sono forward-only:
     * `migrate:rollback`, `migrate:refresh` e `migrate:reset` non si eseguono.
     * Un `down()` che faceva `dropTableIfExists()` significava che un rollback
     * accidentale cancellava la tabella con dentro i dati di produzione.
     *
     * Se una tabella va davvero eliminata, lo si fa a mano, con backup verificato,
     * chiamando esplicitamente {@see static::dropTableIfExists()}.
     *
     * Regola: docs/wiki/rules/dati-sacri.md
     */
